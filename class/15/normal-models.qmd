...
f( x \mid \symbfit{\theta} ) 
 = h(\symbfit{x}) \, \exp\left( \symbfit{\eta}(\symbfit{\theta})^\top \symbfit{T}(\symbfit{x}) - A(\theta) \right) ,
$$
...
\text{Normal}( x \mid \symbfit{\theta} ) 
 &= h(x) \, \exp\left( \symbfit{\eta}(\symbfit{\theta})^\top \symbfit{T}(x) - A(\theta) \right) \\
h(x) &= (2 \pi)^{-\frac{1}{2}} \\
A(\theta) &= \frac{1}{2}\mu^2 \tau - \frac{1}{2} \log \tau \\
...
\text{Normal}( x \mid \symbfit{\eta} ) 
 &= h(x) \, \exp\left( \symbfit{\eta}^\top \symbfit{T}(x) - A(\theta) \right) \\
h(x) &= (2 \pi)^{-\frac{1}{2}} \\
A(\theta) &= 
