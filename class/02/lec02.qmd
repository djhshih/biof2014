...
Trial: Toss a fair coin 3 independent times.
...
However, $P_X(X = x_i) \; = \; 0$ for all $x_i \in \mathcal{X}$ here.

\bigskip

Recall from set theory that
$$
A \subset B \; \Rightarrow \; P(A) \le P(B)  \quad (1)
$$
$$
P(A \cap B) = P(B) - P(B \cap A^c)  \quad (2)
$$
...
For a continuous variable $X$, $P_X(X = x_i) \; = \; 0$ 
for all $x_i \in \mathcal{X}$.

### Proof

By (1), since $\{X = x\} \subset \{x - \epsilon < X \le x \}$ 
for any $\epsilon > 0,$
$$
\begin{aligned}
P\left( X = x \right) \; &\le \; P \left(x - \epsilon < X \le x \right) \\
\; &= \; P \left( \{ r \in \mathcal{X}: x - \epsilon < r \} \, \cap \, \{ r \in \mathcal{X} : r \le x \} \right) \\
\; &= \; P \left( \{ r \in \mathcal{X} : r \le x \} \right)  \\
& \quad - P \left( \{ r \in \mathcal{X} : r \le x \} \, \cap \, \{ r \in \mathcal{X} : r \le x - e \}  \right) \\
\; &= \; P \left( \{ r \in \mathcal{X} : r \le x \} \right) 
 - P \left( \{ r \in \mathcal{X} : r \le x - e \}  \right) \\
