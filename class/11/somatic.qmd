...
We further $B_j$ as the latent indicator random variable such that
$B_j = 1$ if read $j$ is mapped correctly and $B_j = 0$ otherwise.

If read $j$ is mapped correctly ($B_j = 1$), the likelihood is 
similar to the germline variant model:
$$
\begin{aligned}
p \left( x_j \mid Z_j = 1, B_j = 1 \right)
&= \begin{cases}
 q_j & \text{ if } x_j = 1 \\
 1 - q_j & \text{ if } x_j = 0
\end{cases} \\
p \left( x_j \mid Z_j = 0, B_j = 1 \right)
&= \begin{cases}
 1 - q_j & \text{ if } x_j = 1 \\
 q_j & \text{ if } x_j = 0
\end{cases} .
...
If read $j$ is mapped incorrectly ($B_j = 0$), the likelihood is
uniform over the domain of $x_j$:
$$
p \left( x_j \mid Z_j = 1, B_j = 0 \right) = \frac{1}{2},
\quad x_j \in \{0, 1\} .
$$

The prior on the latent mapping indicator $B_j$ is given by
$$
p \left( b_j \right)
= \begin{cases}
 r_j & \text{ if } b_j = 1 \\
...
that we can set to specific values). Hyperapameters are normally treated
as fixed values.
...
$$
p \left( \symbf{\theta} \mid \symbf{\delta} \right)
= \text{Dirichlet}\left( \symbf{\theta} \mid \symbf{\delta} \right) ,
$$
where $\symbf{\delta}$ are hyperparameter weights.
...
Finally, we can define $\symbf{\delta}$ such that more probable
joint genotypes are given higher weights, and $\delta_k$ also
...
2. Assign reasonable values to hyperparamter $\symbf{\delta}$ in the
   somatic model for a single tumour.

3. Using the information in Table 1, assign appropriate values to 
   the hyperparameter $\symbf{\delta}$ in the somatic model for
   tumour-normal pair.

4. If you were to explicitly show the genomic position index $l$,
   which variables would be indexed by $l$?
   
5. If you were to explicity show the sample index $i$,
